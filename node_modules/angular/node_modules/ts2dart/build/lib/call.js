...
var CallTranspiler = (function (_super) {
    ...
    CallTranspiler.prototype.visitNode = function (node) {
        switch (node.kind) {
            ...
            case 158 /* NewExpression */:
                if (this.hasAncestor(node, 130 /* Decorator */)) {
    ...
    CallTranspiler.prototype.visitCall = function (c) {
        if (this.isConstCall(c)) {
            ...
            if (c.arguments.length !== 1)
                this.reportError(c, 'CONST_EXPR takes exactly one argument');
    ...
    CallTranspiler.prototype.isInsideConstExpr = function (node) {
        return this.isConstCall(this.getAncestor(node, 157 /* CallExpression */));
    };
    ...
    CallTranspiler.prototype.handleNamedParamsCall = function (c) {
        ...
        if (last.kind !== 154 /* ObjectLiteralExpression */)
            return false;
        var objLit = last;
        if (objLit.properties.length === 0)
            return false;
        // Even worse: foo(a, b, {'c': d}) is considered to *not* be a named parameters call.
        var hasNonPropAssignments = objLit.properties.some(function (p) { return p.kind != 224 /* PropertyAssignment */ ||
    ...
    CallTranspiler.prototype.visitConstructorBody = function (ctor) {
        ...
        body.statements.forEach(function (stmt) {
            if (stmt.kind !== 182 /* ExpressionStatement */) {
                if (_this.isConst(parent))
                    _this.reportError(stmt, errorAssignmentsSuper);
                return false;
            }
            var nestedExpr = stmt.expression;
            // super() call?
            if (nestedExpr.kind === 157 /* CallExpression */) {
            ...
            if (_this.isConst(parent)) {
                ...
                if (nestedExpr.kind !== 169 /* BinaryExpression */) {
                ...
                // Check for 'this.'
                if (binExpr.left.kind !== 155 /* PropertyAccessExpression */) {
